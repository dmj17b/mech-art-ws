...
class Speech2TextNode(Node):
    ...
    def listener_callback(self, audio_data_msg : Int16MultiArray):
        ...
        transcript = self.get_transcript_from_audio(audio_data_msg.data, audio_data_msg.layout.data_offset)
    ...
    def get_transcript_from_audio(self, recording, sample_rate):
        recording_array = np.array(recording, dtype=np.int16).reshape(-1, 1)

        wav_file_path = '/tmp/latest_audio.wav'
        write(wav_file_path, sample_rate, recording_array)
